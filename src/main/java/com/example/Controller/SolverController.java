...
public class SolverController {
    ...
    public ModelAndView sudokuToTest(@RequestParam String action, @Valid SudokuBox sudokuBox, BindingResult bru) {
        //Gives options if the input is not single digits only
        if (bru.hasErrors()) {
        ...
        //If Sudoku is already solved we do not solve it again
        if (cachedSudoku) {
            if (action.equals("Rensa brädet")) {
                cachedSudoku = false;
                resetBoxes(true);
            ...
            //add potential new input
            setSudokuBox(sudokuBox);

            //Checks if new input is correct, else return info to end user
            if(wrongInput()){
                return new ModelAndView("PuzzelTemplate")
                        .addObject("badBox", "Check your box kid, your input is wrong..")
                        .addObject("box", sudokuBox);
            }

            if (action.equals("Lös min Sudoku!")) {
                return new ModelAndView("PuzzelTemplate")
                        .addObject("box", returnSudokuBox(correctBox));
            }
        ...
        //Tries to solve sudoku give that input is single digits only
        clearBoxes();
        setSudokuBox(sudokuBox);
        markStartingDigits();
        ...
        if (checkCheckBox()) {
            if (sudokuSolver(0, 0)) {
                ...
                for(int i = 0; i < correctBox.length; i++){
                    for(int j = 0; j < correctBox[i].length; j++){
                        correctBox[i][j] = box[i][j];
                    }
                }
        ...
        //Gives feedback if soduko is not solveable
        resetBoxes(false);
        return new ModelAndView("PuzzelTemplate")
                .addObject("badBox", "Check your box kid, it's not correct..")
                .addObject("box", sudokuBox);
